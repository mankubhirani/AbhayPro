...
exports.getScheduler = async (req, res, next) => {
  try {
    const [row_a] = await dbConn.execute(
      "SELECT sch_id AS id, cam_name AS name, cd_subject AS subject, cam_type_id AS type, JSON_UNQUOTE(sch_sent_to) AS sentTo, sch_scheduled_time AS scheduledTimeSt, sch_status AS status FROM `sqm_scheduler` INNER JOIN `sqm_campaign_details` ON sch_campaign_details_id=cd_id INNER JOIN `sqm_campaign` ON cd_campaign_id=cam_id WHERE `sch_user_id`= ? AND sch_is_active=1 AND sch_is_deleted=0",
      [req.body.userId]
    );
    if (row_a.length > 0) {
      return res.json({
