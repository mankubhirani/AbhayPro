...
export class CreateMemberComponent implements OnInit {
    ...
    if (this.memberForm) {
      ...
      memberObs.subscribe(
        (res: any) => {
          ...
          this._router.navigate(['/admin/active-users'])
  ...
  _getRoles() {
    this._memberService._getRolesApi().subscribe((roles: any) => {
      ...
      console.log('roles.data', this.createUserWithRoleId);
      const selectedRole = this.roles.find(role => role.roleId === +this.createUserWithRoleId);

      if (selectedRole) {
          this.setCreationLimit(+this.createUserWithRoleId);
          this.selectedUserRole = selectedRole.userRoleName;
      } else {
          console.error('No matching role found for roleId:', this.createUserWithRoleId);
