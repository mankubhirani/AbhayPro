...
export class InPlayIndexComponent implements OnInit {
  ...
  bannerUrls: string[] = [];
  numImages = environment.bannerCount;
  ...
  async ngOnInit(): Promise<void> {
    ...
    this._sharedService.isMaintenanceActive.subscribe((isMaintenanceActive) => {
      // console.log('isMaintenanceActive',isMaintenanceActive);
    ...
    if (menuList.some(item => item.gameId === 4)) {
      this.isCasinoEnabled = true;
    }

    // for (let i = 1; i <= this.numImages; i++) {
    //   const imageUrl = `https://sportszonegroup9e79.blob.core.windows.net/banners/banner${i}-${this.clientId}.jpg`;
    //   try {
    //     const imageExists = await this._sharedService.checkImageExists(imageUrl);
    //     if (imageExists) {
    //       this.bannerUrls.push(imageUrl);
    //     }
    //   } catch (error) {
    //     console.error('Error checking image:', error);
    //   }
    // }
    // console.log('this.bannerUrls', this.bannerUrls);
  ...
  getBanners(clientId){
    this.http.get('https://apisimg.cylsys.com/get-banners/'+clientId, {})
      .subscribe(response => {
