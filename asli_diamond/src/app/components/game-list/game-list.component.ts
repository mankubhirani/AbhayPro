...
export class GameListComponent implements OnInit {
  ...
  private _updateMarketData(data: any) {
    ...
    if(parseData.hasOwnProperty('data') && typeof parseData?.data !== 'string'){
      ...
      this.matchList= this.inPlayMatchListBySport.map(item => item.sports.filter((sport: any) => sport.sportId === this.sportsId)).flat();
      ...
      const upcomingCricketMatch= this.upComingMatchListBySport.map(item => item.sports.filter((sport: any) => sport.sportId === this.sportsId)).flat();
      //this.matchList = this.matchList.concat(upcomingCricketMatch);
      
      let concatenatedMarkets = this.matchList.map(item => ({
        ...item,
        markets: [...item.markets]
      }));
      
      upcomingCricketMatch.forEach(item => {
        let foundIndex = concatenatedMarkets.findIndex(market => market.sportId === item.sportId);
        if (foundIndex !== -1) {
          concatenatedMarkets[foundIndex].markets.push(...item.markets);
        } else {
          concatenatedMarkets.push({
            ...item,
            markets: [...item.markets]
          });
        }
      });
      this.matchList = concatenatedMarkets;
